...
abstract class TweetSet {
   ...
   * and be implemented in the subclasses?
   */
  //** implement in Empty and NonEmpty
...
//  def mostRetweetedAcc(x:Tweet, acc: TweetSet): Tweet
...
//  def mostRetweetedAcc(x: Tweet, acc: TweetSet): Tweet = x
...
//
//  def mostRetweeted: Tweet = {
//    mostRetweetedAcc(mostRetweetedAcc(elem, left), right)
//  }
//  
//  def mostRetweetedAcc(x:Tweet, acc: TweetSet): Tweet = {
//    if (x.retweets < acc.mostRetweeted.retweets) acc.mostRetweeted
//    else x
//  }
...
object GoogleVsApple {
  ...
  lazy val appleTweets: TweetSet = TweetReader.allTweets.filter(t => google.exists(token => t.text.contains(token)))
